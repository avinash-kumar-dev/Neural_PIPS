...
def train_walk_forward(X, y, ts, n_windows=12, embargo_bars=48):
    ...
    print("  NO class weights — natural distribution")
    t0 = time.time()
...
def test_holdout(xgb, lgbm, catboost, meta_model, pipeline, X_june, y_june, ts_june):
    ...
    results_df.to_csv('data/eurusd/v5p2_holdout_results.csv', index=False)
    print(f"\n  Saved to data/eurusd/v5p2_holdout_results.csv")
...
def save_models(xgb, lgbm, catboost, meta_model, pipeline, wf_summary, holdout_results, train_time, feat_cols, X, y, ts):
    ...
    summary = {
        'description': 'Phase 2: NO class weights, natural distribution, CatBoost, LogisticRegression meta, embargo=48, RobustScaler',
        'train_method': 'walk_forward_12_windows_phase2_no_weights',
...
def main():
    print("=" * 60)
    print("  V5 PHASE 2 TRAINING")
    print("  NO class weights + CatBoost + LogisticRegression + embargo=48")
    print("=" * 60)
    ...
    xgb, lgbm, catboost, meta_model, pipeline = train_final_models(X, y)
    ...
    print("\n" + "=" * 60)
    print("  V5 PHASE 2 TRAINING COMPLETE")
    print("=" * 60)
