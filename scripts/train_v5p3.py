...
def train_walk_forward(X, y, ts, n_windows=12, embargo_bars=48):
    ...
    print("  Meta-learner: weighted probability average (NOT LogisticRegression)")
    t0 = time.time()
    ...
    for i, (train_idx, test_idx) in enumerate(windows):
        ...
        weights = np.array([1.0 / 3.0] * len(base_models))
...
def test_holdout(xgb, lgbm, catboost, weights, pipeline, X_june, y_june, ts_june):
    ...
    results_df.to_csv('data/eurusd/v5p3_holdout_results.csv', index=False)
    print(f"\n  Saved to data/eurusd/v5p3_holdout_results.csv")
...
def save_models(xgb, lgbm, catboost, weights, pipeline, wf_summary, holdout_results, train_time, feat_cols, X, y, ts):
    ...
    summary = {
        'description': 'Phase 3: Optimized weighted average meta (Nelder-Mead), NO class weights, CatBoost, embargo=48, RobustScaler',
        'train_method': 'walk_forward_12_windows_phase3_weighted_average',
...
def main():
    print("=" * 60)
    print("  V5 PHASE 3 TRAINING")
    print("  Optimized weighted average + CatBoost + embargo=48 + RobustScaler")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("  V5 PHASE 3 TRAINING COMPLETE")
    print("=" * 60)
