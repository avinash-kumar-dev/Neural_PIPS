...
MODELS_DIR = 'models/v4_retrained'
...
def generate_features(m5, m15, h1, h4):
    ...
    print(f"  Feature columns: {[c for c in raw_features.columns if c != 'time']}")
...
def generate_labels(m5, raw_features):
    ...
    print(f"  Avg TP: {stats['avg_tp_pips']:.1f} pips, Avg SL: {stats['avg_sl_pips']:.1f} pips")
    print(f"  Avg ATR: {stats['avg_atr_pips']:.1f} pips")
...
def apply_filters(labeled, raw_features):
    ...
    print(f"  Rejection reasons: {stats['reason_counts']}")
...
def prepare_training_data(raw_features, filtered):
    ...
    print(f"  Date range: {pd.Timestamp(ts.min())} to {pd.Timestamp(ts.max())}")
    ...
    print(f"\n  June holdout: {len(X_june):,} bars")
    print(f"  June LONG: {(y_june==1).sum():,}, SHORT: {(y_june==0).sum():,}")
...
def compute_class_weights(y):
    ...
    print(f"\n  Class weights: LONG={weight_long:.2f}, SHORT={weight_short:.2f}")
    return weight_long, weight_short


def train_walk_forward(X, y, ts, n_windows=12):
    print(f"\nWalk-forward training ({n_windows} windows)...")
    t0 = time.time()
    ...
    sample_weight_long = np.where(y == 1, weight_long, weight_short)
    ...
    for i in range(n_windows):
        ...
        if len(train_idx) >= 10000 and len(test_idx) > 0:
    ...
    for i, (train_idx, test_idx) in enumerate(windows):
        ...
        # Compute sample weights for this window
        ...
        lgbm.fit(X_train, y_train, sample_weight=sw_train)

        # Meta-learner
        base_preds = np.column_stack([xgb.predict_proba(X_train)[:, 1], lgbm.predict_proba(X_train)[:, 1]])
        meta_model = Ridge(alpha=1.0)
        meta_model.fit(base_preds, y_train)

        # Predict on test
        test_base = np.column_stack([xgb.predict_proba(X_test)[:, 1], lgbm.predict_proba(X_test)[:, 1]])
        meta_pred = np.clip(meta_model.predict(test_base), 0, 1)
    ...
    print(f"  Precision: {summary['precision_mean']*100:.1f}% (+/- {summary['precision_std']*100:.1f}%)")
    print(f"  Recall: {summary['recall_mean']*100:.1f}% (+/- {summary['recall_std']*100:.1f}%)")
...
def train_final_models(X, y, weight_long, weight_short):
    ...
    meta_features = np.column_stack([xgb.predict_proba(X)[:, 1], lgbm.predict_proba(X)[:, 1]])
    meta_model = Ridge(alpha=1.0)
    ...
    return xgb, lgbm, meta_model


def test_holdout(xgb, lgbm, meta_model, X_june, y_june, ts_june, raw_features, filtered):
    print("\n" + "=" * 60)
    print("  HOLDOUT TEST: June 2026")
    print("=" * 60)

    # Predict on June holdout
    base_preds = np.column_stack([xgb.predict_proba(X_june)[:, 1], lgbm.predict_proba(X_june)[:, 1]])
    ...
    # Rule filters
    june_mask_valid = pd.to_datetime(filtered['time']) >= pd.Timestamp('2026-06-01')
    june_mask_valid = june_mask_valid & (filtered['label'] != -1)
    june_filtered = filtered.loc[june_mask_valid].reset_index(drop=True)
    june_raw = raw_features.loc[june_mask_valid].reset_index(drop=True)

    rf = RuleFilters()
    filtered_june = rf.apply_all(june_filtered, june_raw)
    passed_mask = filtered_june['filter_reject'] == 0

    # ATR filter
    atr_pips = june_raw['atr_pips'].values if 'atr_pips' in june_raw.columns else np.full(len(june_filtered), 10.0)

    # Confidence scores
    from src.confidence_scorer import ConfidenceScorer
    scorer = ConfidenceScorer({'threshold': 0})
    conf_scores = np.zeros(len(june_filtered))

    for i in range(len(june_filtered)):
        if passed_mask.iloc[i]:
            row = june_raw.iloc[i].to_dict()
            row['time'] = june_filtered.iloc[i]['time']
            weighted, _ = scorer.score(row, row)
            conf_scores[i] = weighted

    signal_dir = np.where(ml_pred == 1, 'LONG', 'SHORT')

    print(f"\n  {'T':>4s} {'Dir':>12s} {'Signals':>8s} {'Acc%':>6s}")
    print("  " + "-" * 35)

    results = []
    for threshold in [60, 70, 75, 78, 80, 82, 85]:
        mask = passed_mask.values.astype(bool) & session_mask.astype(bool) & (atr_pips >= 3) & (conf_scores >= threshold)
    ...
    results_df.to_csv('data/eurusd/v4_holdout_results.csv', index=False)
    print(f"\n  Saved to data/eurusd/v4_holdout_results.csv")

    return results_df


def save_models(xgb, lgbm, meta_model, wf_summary, holdout_results, train_time, feat_cols, X, y, ts):
    print("\nSaving models...")
    ...
    summary = {
        'description': 'Retrained with 100K M5 bars, June excluded, class imbalance fixed, H4 hard gate pending.',
        'train_method': 'walk_forward_12_windows_class_weighted',
        ...
        'walk_forward_precision_mean': round(wf_summary['precision_mean'], 4),
        'walk_forward_recall_mean': round(wf_summary['recall_mean'], 4),
        'total_train_bars': int(len(X)),
        'long_count': int((y == 1).sum()),
        'short_count': int((y == 0).sum()),
        'train_time_seconds': round(train_time, 1),
        'holdout_june_metrics': holdout_results.to_dict('records') if holdout_results is not None else None,
    }
...
def main():
    print("=" * 60)
    print("  V4 RETRAINING")
    print("  100K M5 bars, June excluded, class weights fixed")
    print("=" * 60)
    ...
    wf_summary, predictions_df, wf_results, train_time = train_walk_forward(X, y, ts, n_windows=12)

    xgb, lgbm, meta_model = train_final_models(X, y, weight_long, weight_short)

    holdout_results = test_holdout(xgb, lgbm, meta_model, X_june, y_june, ts_june, raw_features, filtered)

    save_models(xgb, lgbm, meta_model, wf_summary, holdout_results, train_time, feat_cols, X, y, ts)
