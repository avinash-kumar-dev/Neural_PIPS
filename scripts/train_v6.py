...
def load_data():
    ...
    raw_features = pd.read_parquet('data/eurusd/features_m5_v6_raw.parquet')

    if 'time' not in raw_features.columns:
        raw_features['time'] = m5['time'].values

    print(f"  M5: {len(m5):,} bars | {m5.time.min()} to {m5.time.max()}")
    print(f"  Features: {raw_features.shape}")
...
def test_holdout(xgb, lgbm, catboost, weights, pipeline, X_june, y_june, ts_june):
    ...
    for threshold in [60, 70, 75, 78, 80, 82, 85]:
        for agreement_thresh in [0.0, 0.2, 0.3]:
            ...
            if agreement_thresh > 0:
                long_mask = long_mask & (ensemble_std <= agreement_thresh)
                short_mask = short_mask & (ensemble_std <= agreement_thresh)

            long_gen = long_mask.sum()
            short_gen = short_mask.sum()
            combined_gen = long_gen + short_gen

            if combined_gen > 0:
                long_labels = y_june[long_mask]
                short_labels = y_june[short_mask]
                long_correct = (long_labels == 1).sum()
                short_correct = (short_labels == 0).sum()
                total_correct = long_correct + short_correct
                acc = total_correct / combined_gen * 100
            else:
                acc = 0
            ...
            results.append({
                ...
                'signals': int(combined_gen),
                'accuracy': round(acc, 1)
            })

            for label, mask, expected_dir in [("LONG-only", long_mask, 1), ("SHORT-only", short_mask, 0)]:
                gen = mask.sum()
                if gen > 0:
                    labels = y_june[mask]
                    correct = (labels == expected_dir).sum()
                    acc_d = correct / gen * 100
                else:
                    acc_d = 0
                results.append({
                    'threshold': threshold,
                    ...
                    'direction': label,
                    'signals': int(gen),
                    'accuracy': round(acc_d, 1)
                })
                if label in ['LONG-only', 'SHORT-only'] and gen > 0 and agreement_thresh in [0.0, 0.3]:
                    print(f"  {threshold:4d} {label:>12s} {gen:8d} {acc_d:6.1f}% {agreement_thresh:10.2f}")

    results_df = pd.DataFrame(results)
    results_df.to_csv('data/eurusd/v6_holdout_results.csv', index=False)
    print(f"\n  Saved to data/eurusd/v6_holdout_results.csv")
...
def save_models(xgb, lgbm, catboost, weights, pipeline, wf_summary, holdout_results, train_time, feat_cols, X, y, ts):
    ...
    summary = {
        'description': 'V6: Phase 2 features (81 total), weighted average meta (Nelder-Mead), CatBoost, embargo=48, RobustScaler',
        'train_method': 'walk_forward_12_windows_v6_weighted_average',
        'train_period': f'{pd.Timestamp(ts.min())} to {pd.Timestamp(ts.max())}',
        'n_features': len(feat_cols),
        'new_features': ['fvg_bull', 'fvg_bear', 'ob_bull', 'ob_bear', 'bos_bull', 'bos_bear',
                        'liquidity_high', 'liquidity_low', 'displacement', 'imbalance', 'premium_discount',
...
def main():
    print("=" * 60)
    print("  V6 TRAINING")
    print("  Phase 2 features (81) + weighted average + CatBoost + embargo=48")
    print("=" * 60)
    ...
    xgb, lgbm, catboost, weights, pipeline = train_final_models(X, y)
